...
import { dashboardOverview } from "@/lib/dashboard-data";
import { cn } from "@/lib/utils";

export function OverviewStats({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("grid sm:grid-cols-2 xl:grid-cols-4 border-t border-l border-border", className)}>
      ...
      <StatCard icon={Clock} label="Avg wait" value={stats.avgWaitMinutes} suffix=" min" />
      <StatCard icon={Hash} label="Current ticket" value={stats.currentTicket} isText />
