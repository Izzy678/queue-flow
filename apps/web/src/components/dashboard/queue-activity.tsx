...
import { dashboardOverview, dashboardQueues, recentTickets } from "@/lib/dashboard-data";
...
export function QueueActivityPanel({ className }: { className?: string }) {
  return (
            ...
            >
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted truncate">{queue.branch}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 text-xs text-muted shrink-0">
                <span>
                  <span className="text-foreground font-medium">{queue.waiting}</span> waiting
                </span>
                <span>
                  <span className="text-foreground font-medium">{queue.serving}</span> serving
                </span>
              </div>
            </div>
          ))}
            ...
            <tbody>
              {recentTickets.map((row) => (
                <tr key={row.ticket} className="border-b border-border last:border-0">
                  <td className="px-3 py-2 font-mono text-xs font-medium">{row.ticket}</td>
                  <td className="px-3 py-2 text-xs hidden sm:table-cell">{row.customer}</td>
                  <td className="px-3 py-2 text-xs text-muted">{row.wait}</td>
  ...
  return (
    <div className={cn("border-t border-l border-border", className)}>
      <div className="border-r border-b border-border p-6">
        ...
        <div className="h-32">
          <MiniChart type="area" data={[...dashboardOverview.throughput]} />
        </div>
      </div>
    </div>
  );
